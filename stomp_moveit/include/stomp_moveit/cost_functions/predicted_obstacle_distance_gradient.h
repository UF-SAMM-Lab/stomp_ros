 ...
 */
class PredictedObstacleDistanceGradient : public StompCostFunction
{
public:
  PredictedObstacleDistanceGradient();
  virtual ~PredictedObstacleDistanceGradient();
...
protected:
  ...
  double checkIntermediatePredictedCollisions(const Eigen::VectorXd& start,
  ...
  std::vector<std::tuple<float,std::vector<fcl::Cylinderf>,std::vector<fcl::Transform3f>>> human_seq;
  ...
  double min_separation = 0.2;
  double max_separation = 0.5;
  double human_dt = 0.1;
  int dof=6;
  Eigen::VectorXd max_velocity_;
  Eigen::VectorXd inv_max_speed_;
  std::vector<std::string> joint_names_;
  double stomp_delta_t = 1.0;
};
